...
% This example shows how to work with arrays having more than two
% dimensions. Multidimensional arrays can be numeric, character, cell, or
% structure arrays.
% 
% Multidimensional arrays can be used to represent multivariate data.
% MATLAB(R) provides a number of functions that directly support
...
% subscripts. For example A(1,2,2), using A defined above, returns 0.
% 
...
% arrays. RESHAPE behaves as it does for 2D arrays. The operation of
% PERMUTE is illustrated below.
% 
% Let A be a 3 by 3 by 2 array. PERMUTE(A,[2 1 3]) returns an array with
% the row and column subscripts reversed (dimension 1 is the row, dimension
% 2 is the column, dimension 3 is the depth and so on). Similarly,
% PERMUTE(A,[3,2,1]) returns an array with the first and third subscripts
% interchanged.
...
%% Selecting 2D Matrices From Multidimensional Arrays
% Functions like EIG that operate on planes or 2D matrices do not accept
% multidimensional arrays as arguments. To apply such functions to
...
% gridding functions that operate specifically on multidimensional data.
% Here is an example of NDGRID applied to an N-dimensional matrix.
...
% arrays, and can also convert between multidimensional numeric and cell
% arrays.
% 
