...
c=deblank(c);
prodNameDemosString = getString(message('MATLAB:demos:shared:TitleProdNameDemos',prodName));
header=sprintf('<title>%s</title><h1>%ss</h1>', prodNameDemosString, prodNameDemosString);
inList=0;
for i=1:length(c);
    line=c{i};
    line=strrep(line,'>','&gt;');
